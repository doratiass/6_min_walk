...
raw_df <- read_excel("data/CHF_final_wide.xlsx") %>%
  ...
  mutate(
    x6mw_dist_meter = ifelse(
      x6mw_dist_meter > 800 | x6mw_dist_meter < 10,
      NA,
      x6mw_dist_meter
    ),
    x6mw_hr_pre = ifelse(x6mw_hr_pre > 300 | x6mw_hr_pre < 40, NA, x6mw_hr_pre),
    x6mw_hr_post = ifelse(
      x6mw_hr_post > 300 | x6mw_hr_post < 40,
      NA,
      x6mw_hr_post
    ),
    x6mw_sat_pre = ifelse(
      x6mw_sat_pre > 100 | x6mw_sat_pre < 60,
      NA,
      x6mw_sat_pre
    ),
    x6mw_sat_post = ifelse(
      x6mw_sat_post > 100 | x6mw_sat_post < 60,
      NA,
      x6mw_sat_post
    ),
    ...
    kupa = factor(case_when(
      kupa %in%
        ...
        "Clalit",
      kupa %in% c("מכבי", "שב\"ן מכבי זהב", "קופת חולים מכבי") ~ "Maccabi",
    ...
    weight_category = factor(case_when(
      weight_category %in% c("תקין", "טוב", "סביר", "תקין (מעט רזה)") ~
        "Normal",
      weight_category %in%
        c(
          "רזה",
          ...
          "מעט רזה",
          "ירוד",
          "רזה (ירד במשקל לאחר התקף הלב להערכתו מעל 10 ק\"ג)"
        ) ~
        "Thin",
    ...
    pillows_reason_dyspnea = grepl(
      "BIPAP|CIPAP|CPAP|נשימה|חמצן|משתעל",
      pillows_reason
    ),
    ...
    sglt2 = grepl(
      "Jardiance|Empagliflozin|Forxiga|Dapagliflozin|Xigduo|Glyxambi|Empagliflozin",
      medications,
      ignore.case = TRUE
    ),
    glp1 = grepl(
      "Victoza|Trulicity|Ozempic|Saxenda|Xultophy|Liraglutide|Dulaglutide|Semaglutide",
      medications,
      ignore.case = TRUE
    ),
    fusid = grepl(
      "\\b[Ff]usid\\b|\\b[Ff]urosemide\\b",
      medications,
      ignore.case = TRUE
    ),
    torsemide = grepl(
      "\\bTorsemide\\b|\\bTorsemid\\b",
      medications,
      ignore.case = TRUE
    ),
    bumetanide = grepl("\\bBumetanide\\b", medications, ignore.case = TRUE),
    metolazone = grepl(
      "\\bMetolazone\\b|\\bZaroxylin\\b|\\bZaroxillin\\b|\\bZaroxyllyn\\b",
      medications,
      ignore.case = TRUE
    ),
    k_spare = grepl(
      "\\bSpironolactone\\b|\\bAldactone\\b|\\bAldospirone\\b|\\bEplerenone\\b|\\bInspra\\b",
      medications,
      ignore.case = TRUE
    ),
    thiazide = grepl(
      "\\bHydrochlorothiazide\\b",
      medications,
      ignore.case = TRUE
    ),
    allopurinol = grepl(
      "\\bAllopurinol\\b|\\bAlloril\\b|\\bZylol\\b|\\bZylori\\b",
      medications,
      ignore.case = TRUE
    ),
    beta_blocker = grepl(
      "Bisoprolol|Concor|Cardiloc|Metoprolol|Lopresor|Neobloc|Metopress|Carvedilol|Dimitone|Atenolol|Normiten|Normalol|Propranolol|Deralin|Prolol",
      medications,
      ignore.case = TRUE
    ),
    anticoagulant = grepl(
      "Xarelto|Rivaroxaban|Coumadin|Warfarin|Eliquis|Apixaban|Pradaxa|Dabigatran|Sintrom|Acenocoumarol",
      medications,
      ignore.case = TRUE
    ),
    antiplatlet = grepl(
      "Plavix|Clopidogrel|Brilinta|Ticagrelor|Effient|Prasugrel|Aspirin|Acetylsalicylic|Cardioaspirin|Cardiopirin|Aspocid|Aspicor|Aspimax|Aspirin",
      medications,
      ignore.case = TRUE
    ),
    ace_inhibitor = grepl(
      "Tritace|Enalapril|Ramipril|Lisinopril|Captopril|Perindopril|Quinapril|Trandolapril|Moexipril|Benazepril",
      medications,
      ignore.case = TRUE
    ),
    arbs = grepl(
      "Losartan|Ocsaar|Valsartan|Diovan|Irbesartan|Candesartan|Atacand|Olmesartan|Telmisartan|Eprosartan|Azilsartan",
      medications,
      ignore.case = TRUE
    ),
    arni = grepl(
    ...
    insulin = grepl(
      "Lantus|Levemir|Toujeo|Novolog|Humalog|Apidra|Fiasp|Novorapid|Humulin|Novolin|Tresiba|Basaglar",
      medications,
      ignore.case = TRUE
    ),
    metformin = grepl(
      "Metformin|Glucophage|Glucovance|Janumet|Jentadueto|Kombiglyze|Synjardy|Xigduo|Invokamet|Qtern|Steglatro|Segluromet|Glyxambi|Steglujan",
      medications,
      ignore.case = TRUE
    ),
    dpp4 = grepl(
      "Januvia|Onglyza|Tradjenta|Nesina|Galvus|Zomelis|Qtern|Steglatro|Segluromet|Glyxambi|Steglujan",
      medications,
      ignore.case = TRUE
    ),
    glinide = grepl(
      "Starlix|Prandin|Novonorm",
      medications,
      ignore.case = TRUE
    ),
    sulfonylurea = grepl(
      "Glibenclamide|Glyburide|Glimepiride|Gliclazide|Tolbutamide|Diamicron|Amaryl|Daonil",
      medications,
      ignore.case = TRUE
    ),
    ...
    family_history = case_when(
      family_history %in%
        c(
          ...
          "אין",
          "ללא",
          "לא ידוע על איס\"ל במשפחה.",
          "שולל מחלת לב במשפחה",
          "שולל מחלות לב או מוות פתאומי",
          "שולל מחלת לב במשפחה.",
          "ללא רקע משפחתי",
          "לא ידועיש יל\"ד משפחתי.",
          "לאלאב היתה שחפת ככל הנראה",
          "לא ידוע (ניצול שואה)",
          "שוללת",
          "ללא סיפור משפחתי",
          "ללא חולי לב במשפחה",
          "הורים נפטרו משחפת.",
          "שולל מחלות לב במשפחה",
          "ללא רקע"
        ) ~
        FALSE,
      TRUE ~ !is.na(family_history)
    ),
    family_history_detail = ifelse(
      family_history,
    ...
    hosp = case_when(
      hosp_detail %in%
        c(
          ...
          "לאלא",
          "לא;",
          "לא. עבר ניתוח של טריגר פינגר באשפוז יום",
          "לא.עקירה כירורגית של שן",
          "לא רלוונטי",
          "לא (אלקטיבי)",
          "לא - למעט מרפאה המטולוגית",
          "לא (אלקטיבי לICD)",
          "לא (אשפוז יום).",
          "לא (היה מאושפז לפני כ8 חודשים בקרדיולוגיה ועבר השתלת ICD)",
          "לא (למעט אלקטיבי)",
          "לא (למעט לביופסיה)",
          "לא (מטופל באשפוז יום המטולוגי)",
          "לא (שדרוג לCRTD אלקטיבי.",
          "לא בארבע השנים האחרונות",
          "לא היה אשפוז",
          "לא למעט אשפוז יום",
          "לא למעט ניתוח",
          "לא מעבר לאשפוז יום",
          "לא. היתה מאושפזת לפני שנתיים בקרדיולוגיה,  אז אבחנו לה CHF"
        ) ~
        FALSE,
...
clean_df_identified <- raw_df %>%
  ...
  mutate(
    ...
    last_fup_date = case_when(
      ...
      mortality_status == 1 & time_diff_months > max_fup_time ~
        first_visit + months(max_fup_time),
      mortality_status == 0 ~
        pmin(fup_date, first_visit + months(max_fup_time), na.rm = TRUE)
    ),
