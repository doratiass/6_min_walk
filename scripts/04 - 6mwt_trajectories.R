...
time_id <- long_df %>%
  filter(time <= tests_time, fup_time >= tests_time) %>% # Select records for the given subject before t0
  mutate(
...
quan <- new_df %>%
  group_by(id, gender) %>%
  summarise(
    change = last(pred_x6mw_dist_meter) - first(pred_x6mw_dist_meter)
  ) %>%
  ungroup() %>%
  group_by(gender) %>%
  mutate(
...
# -------------------------------------------------------------------------- #
ggarrange(
  pred_6mwt_plot + theme(legend.position = "none"),
  km_plot$plot +
    theme(
...
annotate_figure(
  ...
  #                 face = "bold", size = 16),
  left = text_grob("6 minute walk distance (meters)", rot = 90, size = 12),
  right = text_grob("Predicted Probability of Mortality", rot = 270, size = 12),
  bottom = text_grob("Time (months)", rot = 0, vjust = -0.5, size = 12)
)
