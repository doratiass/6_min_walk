...
missing_ids_full <- clean_df %>%
  filter(if_any(all_of(seattle_vars), is.na)) %>%
  distinct(id) %>%
  pull(id)

# Print the number and percentage of patients with missing values.
print(paste0("Number of patients with missing values: ", length(missing_ids_full), 
             " (", round(length(missing_ids_full) / total_ids * 100, 2), "%) out of ",
             total_ids, " patients"))
...
long_df_full <- clean_df %>%
  ...
  group_by(id, fup_time, mortality_status, time, !!!syms(seattle_vars)) %>%
  summarise(
...
lme_6min_full <- lme(x6mw_dist_meter ~ ns(time, df = 3) * (age + gender) + nyha, 
                data = long_df_full, 
                random = ~ ns(time, df = 3) | id, 
                control = lmeControl(opt = 'optim'))

# Fit the same model on the subset of patients with more than one visit.
lme_6min_full_3 <- lme(x6mw_dist_meter ~ ns(time, df = 3) * (age + gender) + nyha, 
                  data = long_df_full_3, 
                  random = ~ ns(time, df = 3) | id, 
                  control = lmeControl(opt = 'optim'))
...
CoxFit_small <- coxph(Surv(fup_time, mortality_status) ~ gender + age + nyha, 
                data = cox_df_full, 
                model = TRUE, x = TRUE, y = TRUE)

CoxFit_small_3 <- coxph(Surv(fup_time, mortality_status) ~ gender + age + nyha, 
                  data = cox_df_full_3, 
                  model = TRUE, x = TRUE, y = TRUE)

# Fit a full baseline Cox model using gender, age, and NYHA.
CoxFit_full <- coxph(as.formula(paste("Surv(fup_time, mortality_status) ~", 
                                      paste(seattle_vars, collapse = " + "))), 
                     data = cox_df_full, 
                     model = TRUE, x = TRUE, y = TRUE)

CoxFit_full_3 <- coxph(as.formula(paste("Surv(fup_time, mortality_status) ~", 
                                        paste(seattle_vars, collapse = " + "))), 
                       data = cox_df_full_3, 
                       model = TRUE, x = TRUE, y = TRUE)
...
jointFit_small <- jm(CoxFit_small, lme_6min_full, time_var = "time",
               functional_forms = ~ value(x6mw_dist_meter) + slope(x6mw_dist_meter))

# Joint model on the subset of patients with >1 visit.
jointFit_small_3 <- jm(CoxFit_small_3, lme_6min_full_3, time_var = "time",
                 functional_forms = ~ value(x6mw_dist_meter) + slope(x6mw_dist_meter))

# Final joint model (using CoxFit) with value + slope specification.
jointFit_full <- jm(CoxFit_full, lme_6min_full, time_var = "time",
                    functional_forms = ~ value(x6mw_dist_meter) + slope(x6mw_dist_meter))

# Joint model on the subset of patients with >1 visit.
jointFit_full_3 <- jm(CoxFit_full_3, lme_6min_full_3, time_var = "time",
                      functional_forms = ~ value(x6mw_dist_meter) + slope(x6mw_dist_meter))
...
plot_boot_df(boot_eval_full_summary, 
             model_filter = c("Full Joint Model","Joint Model"),
             metric_filter = "AUC", 
             y_lab = "AUC",
             plot_title = "",
             plot_subtitle = "",
             hline_position = 0.5) -> auc_plot_full
...
plot_boot_df(boot_eval_full_summary, 
             ...
             plot_title = "",
             plot_subtitle = "",
             hline_position = 0) -> brier_plot_full

ggarrange(auc_plot_full  +
            ggtitle("AUC") +
...
ggsave(file.path("export", "supp_fig_1.jpeg"), last_plot(), 
